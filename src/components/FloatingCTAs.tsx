...
export default function FloatingCTAs() {
  ...
  return (
    <>
      {/* Floating Buttons on Right (Desktop Only to prevent mobile touch target overlap) */}
      <div className="hidden sm:flex fixed bottom-6 right-5 z-40 flex-col space-y-3 items-end">
        {/* WhatsApp Floating */}
        <motion.a
          href={whatsappUrl}
          target="_blank"
          ...
          whileTap={{ scale: 0.9 }}
          className="flex items-center justify-center w-14 h-14 bg-emerald-600 hover:bg-emerald-700 text-white rounded-full shadow-lg cursor-pointer transition-colors duration-200 animate-pulse-subtle relative group"
      ...
      <div className="sm:hidden fixed bottom-0 left-0 w-full bg-slate-900 border-t border-slate-800 shadow-[0_-4px_12px_rgba(0,0,0,0.3)] grid grid-cols-2 p-3 gap-3 z-50">
        <a
          href="tel:+918121488961"
          onClick={() => trackGoogleConversion()}
          className="flex items-center justify-center space-x-2 bg-[#ea580c] hover:bg-[#c2410c] text-white py-3.5 px-4 rounded-xl text-base font-extrabold active:scale-95 transition-transform min-h-[48px]"
          aria-label="Call Priyanka Enterprises"
        >
        ...
        <a
          ...
          rel="noopener noreferrer"
          onClick={() => trackGoogleConversion()}
          className="flex items-center justify-center space-x-2 bg-emerald-600 hover:bg-emerald-700 text-white py-3.5 px-4 rounded-xl text-base font-extrabold active:scale-95 transition-transform min-h-[48px]"
          aria-label="WhatsApp Priyanka Enterprises"
        >
